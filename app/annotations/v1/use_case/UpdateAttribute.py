...
class UpdateAttribute:
    ...
    async def execute(self, user_id: str, attribute_name: str, new_attribute_name: str):
        user = await self.user_service.get(user_id)
        client: Client = user.client
        
        attribute: Attribute = await self.attribute_service.get_by_name_and_client_id(attribute_name, client.id, should_exist=True)
        await self.attribute_service.get_by_name_and_client_id(new_attribute_name, client.id, should_exist=False)
        
        attribute.name = new_attribute_name
        await self.attribute_service.update(attribute.id, attribute.model_dump())
