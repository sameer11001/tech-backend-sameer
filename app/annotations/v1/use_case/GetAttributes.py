...
class GetAttributes:
    ...
    async def execute(self, user_id: str, limit: int, page: int, search: str):
        user = await self.user_service.get(user_id)
        client: Client = user.client
        
        result = await self.attribute_service.get_by_client_id_and_search(client.id, page, limit, search)
